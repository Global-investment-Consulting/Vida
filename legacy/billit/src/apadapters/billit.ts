...
async function resolveRegistrationId(config: BillitBaseConfig, auth: AuthHeaders): Promise<string> {
  ...
  const headerVariants: Array<{ headers: Record<string, string>; label: string }> = [
    { headers: auth.headers, label: auth.mode }
  ];
  ...
  if (auth.mode === "api-key" && config.clientId && config.clientSecret) {
    try {
      ...
      // ignore oauth fallback errors; continue with available headers
    }
  }
...
function collectCandidateIds(record: Record<string, unknown>): string[] {
  ...
  for (const value of Object.values(record)) {
    ...
    if (Array.isArray(value)) {
      for (const entry of value) {
        if (entry && typeof entry === "object" && !Array.isArray(entry)) {
          for (const nested of collectCandidateIds(entry as Record<string, unknown>)) {
    ...
    if (typeof value === "object") {
      for (const nested of collectCandidateIds(value as Record<string, unknown>)) {
        if (!candidates.includes(nested)) {
          candidates.push(nested);
        }
      }
...
function describeRegistrationPayload(payload: unknown): string {
  if (Array.isArray(payload)) {
    ...
    if (Array.isArray(value) && value.length > 0) {
      ...
      const nestedKeys =
        nested && typeof nested === "object" && !Array.isArray(nested)
          ? Object.keys(nested as Record<string, unknown>).slice(0, 8)
          : [];
